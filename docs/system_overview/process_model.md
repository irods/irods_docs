...
Here is another example showing three Agents handling client requests. From this, we know there are three active connections to the server. Additional information about each connection can be viewed through the use of [`ips`](../icommands/user/#ips).
...
The following table outlines the expected behavior for each process.

| Process                | Expected Duration        | Expected Memory Usage           |
| ---------------------- | ------------------------ | ------------------------------- |
| irodsServer            | Long-running             | ~60.2 MB                        |
| irodsAgent _(factory)_ | Long-running             | ~120.5 MB                       |
| irodsAgent             | Depends on client        | ~82.3 MB                        |
| irodsDelayServer       | Long-running             | ~53 MB                          |

Note that the duration of Agent processes depends on the client. Clients manage connections in ways that best suit their needs. Some may use connection pooling, while others may create and tear down connections as required.
